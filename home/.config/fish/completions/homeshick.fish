# Homeshick Tab Completion
# Upstream: https://github.com/andsens/homeshick/raw/master/completions/homeshick.fish
...
# NOTE: Use `command find` to use normal `find` (cuz I aliased `find` to `fd`)
function __fish_homeshick_list_castles
    set repos "$HOME/.homesick/repos"
    for repo in (command find -L $repos -mindepth 2 -maxdepth 2 -type d -name .git -exec dirname \{\} \;)
        basename $repo
    end
end
